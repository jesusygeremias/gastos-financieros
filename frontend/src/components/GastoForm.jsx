...
export default function GastoForm({ cuentas, addGasto }) {
    ...
    const cargarPorDefecto = () => {
        ...
        const cuentaHipoteca = cuentas.find(c => c.banco === "Openbank" && c.tipoCuenta === "Hipoteca");
        if (cuentaHipoteca) defaults.push({ descripcion: "Hipoteca", monto: 690, categoria: "Vivienda", cuentaId: cuentaHipoteca.id });
        const cuentaAhorro = cuentas.find(c => c.banco === "Revolut" && c.tipoCuenta === "Ahorro");
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-gradient-to-br from-white to-gray-50 p-6 rounded-2xl shadow-xl hover:shadow-2xl transition duration-300 mb-8 border border-gray-100">
            ...
            <div className="flex flex-wrap gap-4 mt-2">
                <button type="button" onClick={addFila} className="bg-gray-300 px-5 py-2 rounded-lg hover:bg-gray-400 shadow-sm transition">
                    Añadir fila
                </button>
                <button type="button" onClick={cargarPorDefecto} className="bg-purple-500 text-white px-5 py-2 rounded-lg hover:bg-purple-600 shadow-lg transition">
                    Cargar por defecto
                </button>
                <button type="submit" className="bg-blue-500 text-white px-5 py-2 rounded-lg hover:bg-blue-600 shadow-lg transition">
                    Guardar
                </button>
            </div>
        </form>
